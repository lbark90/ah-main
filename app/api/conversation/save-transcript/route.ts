...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      // Create a unique file path in the user's conversations folder
      ...
      return NextResponse.json({ 
        error: 'Failed to save transcript to storage',
...
export async function POST(request: Request) {
  try {
    ...
    // Create folder structure: userId/conversations/
    const folderPath = `${userId}/conversations`;
    const fileName = `transcript_${timestamp.replace(/:/g, '-')}.txt`;
    const filePath = `${folderPath}/${fileName}`;

    console.log(`Saving transcript to ${filePath}`);

    // Upload transcript to GCP bucket
    const bucket = storage.bucket(bucketName);
    const file = bucket.file(filePath);
